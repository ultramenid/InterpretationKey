L.TileLayer.BetterWMS = L.TileLayer.WMS.extend({
    ...
    showGetFeatureInfo: function (err, latlng, content) {
      var papua = document.getElementById('papua');
      if (content) {// do nothing if there's an error
      
      // Otherwise show the content in a popup, or something.
      var pulau = content.features[0].properties.pulau;
      if(pulau == 'Papua'){
        papua.style.display = "block"
      }else if(pulau == 'Jawa Bali'){
        papua.style.display = "block"
      }else if(pulau == 'Sulawesi'){
        papua.style.display = "block"
      }else if(pulau == 'Kalimantan'){
        papua.style.display = "block"
      }else if(pulau == 'Sumatra'){
        papua.style.display = "block"
      }else if(pulau == 'Nusa Tenggara'){
        papua.style.display = "block"
      }else if(pulau == 'Maluku'){
        papua.style.display = "block"
      }
    }
  }
